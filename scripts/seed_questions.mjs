...
public class Main {
    static String checkPacket(String s) {
        ...
        for (char c : s.toCharArray()) {
            ...
            else if (c == ')') { if (stack.isEmpty() || stack.pop() != '(') return "INVALID"; }
            else if (c == '}') { if (stack.isEmpty() || stack.pop() != '{') return "INVALID"; }
            else if (c == ']') { if (stack.isEmpty() || stack.pop() != '[') return "INVALID"; }
        }
...
async function seed() {
  ...
  const { rows: sessions } = await client.query(
    "SELECT id, label FROM contest_sessions ORDER BY created_at ASC LIMIT 1"
  );
  ...
  console.log(`Seeding questions for session: ${sessions[0].label} (${sessionId})...`);
  ...
  for (let i = 0; i < DEFAULT_QUESTIONS.length; i++) {
    ...
    const { rows: insertedQ } = await client.query(
      `INSERT INTO questions (
        session_id, title, category, difficulty, points, display_order,
        scenario, input_format, output_format, constraints,
        starter_c, starter_python, starter_java, is_active
      ...
      [
        ...
        q.outputFormat,
        q.constraints,
        q.starterTemplates?.c || '',
        q.starterTemplates?.python || '',
        q.starterTemplates?.java || '',
      ]
    );

    const questionId = insertedQ[0].id;
    console.log(`  Added question: ${q.title} (${questionId})`);

    for (let j = 0; j < q.testCases.length; j++) {
      const tc = q.testCases[j];
      await client.query(
        `INSERT INTO test_cases (
          question_id, input, expected_output, is_hidden, explanation, display_order
        ) VALUES ($1, $2, $3, $4, $5, $6)`,
        [
          questionId,
          tc.input,
